...
int main(int argc, char** argv) {
    ...
    int buf[5] = {grid_size, repetitions, check_termination, print, readfile};
    MPI_Bcast(buf, 5, MPI_INT, 0, MPI_COMM_WORLD);
    grid_size         = buf[0];
    repetitions       = buf[1];
    check_termination = buf[2];
    print             = buf[3];
    readfile          = buf[4];
    ...
    for (rep = 0; rep < repetitions; rep++) {
         ...
         */
        if (subgrid_size > 2) {
            count_alive(&local_grid1, local_grid2, subgrid_size + 2, INNER);          
        }
         ...
         */
        if (subgrid_size > 2) {
            count_alive(&local_grid1, local_grid2, subgrid_size + 2, OUTTER);
        } else if (subgrid_size == 2) {
            count_alive(&local_grid1, local_grid2, subgrid_size + 2, ALL);
        }
